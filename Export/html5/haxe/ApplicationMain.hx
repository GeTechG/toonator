...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("ToonatorEditor", create);

		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed("ToonatorEditor", null, 0, 0);
		#end
		#else
		create(null);
		#end
	...
	public static function create(config):Void
	{
		...
		app.meta["build"] = "1";
		app.meta["company"] = "kr0tyara";
		app.meta["file"] = "ToonatorEditor";
		app.meta["name"] = "ToonatorEditor";
		app.meta["packageName"] = "ru.kr0tyara.toonator";
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("Setup")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess Setup.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"Setup\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"Setup\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends Setup {}
#else
class DocumentClass
{
